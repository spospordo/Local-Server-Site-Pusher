    ...
    const devices = trackedDevices.map(device => {
      ...
      if (!entityState) {
        return {
          ...
          state: 'unavailable',
          lastUpdated: null,
          connectionError: `Entity '${device.entityId}' was not found in Home Assistant`,
      ...
      if (rawState === 'unavailable' || rawState === 'unknown') {
        return {
          ...
          state: rawState,
          lastUpdated,
          connectionError: `Entity '${device.entityId}' reported state '${rawState}' — the device may be offline or unreachable`,
          lowBatteryThreshold: device.lowBatteryThreshold || 25,
          showWhenCharging: device.showWhenCharging !== false,
          showWhenFull: device.showWhenFull !== false,
          showWhenLow: device.showWhenLow !== false
