...
// NFS Network Drive API Endpoints
// Check if NFS is supported
app.get('/admin/api/nfs/supported', requireAuth, (req, res) => {
  try {
    const result = nfs.isNfsSupported();
    res.json(result);
  } catch (err) {
    res.status(500).json({ 
      supported: false, 
      error: 'Failed to check NFS support: ' + err.message 
    });
  }
});

// Get all NFS connections
app.get('/admin/api/nfs/connections', requireAuth, (req, res) => {
  try {
    const connections = nfs.getConnections();
    res.json({ success: true, connections });
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to get NFS connections: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to get connections: ' + err.message });
  }
});

// Get a specific NFS connection
app.get('/admin/api/nfs/connections/:id', requireAuth, (req, res) => {
  try {
    const connection = nfs.getConnection(req.params.id);
    if (connection) {
    ...
    logger.error(logger.categories.SYSTEM, `Failed to get NFS connection: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to get connection: ' + err.message });
  }
});

// Test NFS connection
app.post('/admin/api/nfs/test', requireAuth, async (req, res) => {
  try {
    const connection = req.body;
    logger.info(logger.categories.SYSTEM, `Testing NFS connection: ${connection.name}`);
    
    const result = await nfs.testConnection(connection);
    res.json(result);
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `NFS test failed: ${err.message}`);
    res.status(500).json({ success: false, error: 'Test failed: ' + err.message });
  }
});

// Add new NFS connection
app.post('/admin/api/nfs/connections', requireAuth, (req, res) => {
  try {
    const connection = req.body;
    logger.info(logger.categories.SYSTEM, `Adding NFS connection: ${connection.name}`);
    
    const result = nfs.addConnection(connection);
    if (result.success) {
      res.json(result);
    } else {
      res.status(400).json(result);
    ...
    logger.error(logger.categories.SYSTEM, `Failed to add NFS connection: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to add connection: ' + err.message });
  }
});

// Update NFS connection
app.put('/admin/api/nfs/connections/:id', requireAuth, (req, res) => {
  try {
    const updates = req.body;
    logger.info(logger.categories.SYSTEM, `Updating NFS connection: ${req.params.id}`);
    
    const result = nfs.updateConnection(req.params.id, updates);
    if (result.success) {
      res.json(result);
    } else {
      res.status(400).json(result);
    ...
    logger.error(logger.categories.SYSTEM, `Failed to update NFS connection: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to update connection: ' + err.message });
  }
});

// Delete NFS connection
app.delete('/admin/api/nfs/connections/:id', requireAuth, async (req, res) => {
  try {
    logger.info(logger.categories.SYSTEM, `Deleting NFS connection: ${req.params.id}`);
    
    const result = await nfs.deleteConnection(req.params.id);
    if (result.success) {
      res.json(result);
    } else {
      res.status(400).json(result);
    ...
    logger.error(logger.categories.SYSTEM, `Failed to delete NFS connection: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to delete connection: ' + err.message });
  }
});

// Mount NFS connection
app.post('/admin/api/nfs/connections/:id/mount', requireAuth, async (req, res) => {
  try {
    logger.info(logger.categories.SYSTEM, `Mounting NFS connection: ${req.params.id}`);
    
    const result = await nfs.mountConnection(req.params.id);
    res.json(result);
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to mount NFS: ${err.message}`);
    res.status(500).json({ success: false, error: 'Mount failed: ' + err.message });
  }
});

// Unmount NFS connection
app.post('/admin/api/nfs/connections/:id/unmount', requireAuth, async (req, res) => {
  try {
    logger.info(logger.categories.SYSTEM, `Unmounting NFS connection: ${req.params.id}`);
    
    const result = await nfs.unmountConnection(req.params.id);
    res.json(result);
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to unmount NFS: ${err.message}`);
    res.status(500).json({ success: false, error: 'Unmount failed: ' + err.message });
  }
});

// Upload backup to NFS
app.post('/admin/api/nfs/connections/:id/upload-backup', requireAuth, async (req, res) => {
  try {
    const connectionId = req.params.id;
    const { filename } = req.body;
    
    logger.info(logger.categories.SYSTEM, `Uploading backup to NFS: ${connectionId}`);
    
    // Export backup data
    const exportResult = backup.exportAllData(config);
    if (!exportResult.success) {
      return res.status(500).json({ success: false, error: exportResult.error });
    }
    
    // Upload to NFS
    const uploadResult = await backup.uploadToNFS(connectionId, exportResult.backup, filename);
    res.json(uploadResult);
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to upload backup to NFS: ${err.message}`);
    res.status(500).json({ success: false, error: 'Upload failed: ' + err.message });
  }
});

// List backups on NFS
app.get('/admin/api/nfs/connections/:id/backups', requireAuth, async (req, res) => {
  try {
    const connectionId = req.params.id;
    logger.info(logger.categories.SYSTEM, `Listing backups on NFS: ${connectionId}`);
    
    const result = await backup.listNFSBackups(connectionId);
    res.json(result);
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to list NFS backups: ${err.message}`);
    res.status(500).json({ success: false, error: 'Failed to list backups: ' + err.message });
  }
});

// Download and restore backup from NFS
app.post('/admin/api/nfs/connections/:id/restore-backup', requireAuth, async (req, res) => {
  try {
    const connectionId = req.params.id;
    ...
    logger.info(logger.categories.SYSTEM, `Restoring backup from NFS: ${remotePath}`);
    
    // Download from NFS
    const downloadResult = await backup.downloadFromNFS(connectionId, remotePath);
    if (!downloadResult.success) {
      return res.status(500).json(downloadResult);
    }
    
    // Import the backup
    const importResult = backup.importAllData(downloadResult.backup, config);
    ...
    if (importResult.success) {
      ...
      try {
        ...
        logger.info(logger.categories.SYSTEM, 'Configuration reloaded after NFS restore');
      } catch (reloadErr) {
        logger.warning(logger.categories.SYSTEM, `Config reload warning: ${reloadErr.message}`);
      }
      ...
      res.json({
        success: true,
        message: 'Backup restored from NFS successfully',
        results: importResult.results,
        warnings: importResult.warnings
      });
    } else {
      res.status(400).json(importResult);
    }
  } catch (err) {
    logger.error(logger.categories.SYSTEM, `Failed to restore backup from NFS: ${err.message}`);
